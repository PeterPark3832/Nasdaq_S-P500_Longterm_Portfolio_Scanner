...
def _cmd_status(chat_id: str) -> None:
    ...
    for h in sorted(stock_holdings, key=lambda x: -x["weight"]):
        if total_val > 0:
            ...
            drift_tag = f" ⚠️{drift:+.1f}%p" if abs(drift) >= threshold else f" ({drift:+.1f}%p)"
        else:
...
def _cmd_help(chat_id: str) -> None:
    ...
    _reply_to(
        ...
        "🤖 *미국주식 스캐너 명령어 목록*\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "/status — 수익률 + 낙폭 + 종목별 비중 드리프트 즉시 조회\n"
...
if __name__ == "__main__":
    ...
    log.info("✅ 미국주식 장기 투자 스캐너 v4.10 시작")
