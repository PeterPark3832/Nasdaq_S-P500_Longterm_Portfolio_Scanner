...
def build_rebalancing_brief(
    ...
    vix_level: float = 0.0,
    prev_portfolio: dict | None = None,   # [v4.6] 체크리스트용 이전 포트폴리오
    ...
    checklist_body = (
        "\n━━━━━━━━━━━━━━━━━━\n"
        + build_trade_checklist(prev_portfolio, new_holdings)
        + "\n"
    )
...
def check_mdd_alert(current_ret: float) -> None:
    """성과 이력 기반 MDD 경보.

    과거 기록된 포트폴리오 수익률 중 최고점(peak) 대비
    현재 수익률 낙폭이 mdd_alert_threshold 이하이면 텔레그램 경보.
    """
    records = load_performance_history()
    if not records:
        return
    past_rets = [r["portfolio_ret_pct"] for r in records if r.get("portfolio_ret_pct") is not None]
    if not past_rets:
        return
    peak      = max(past_rets + [current_ret])
    drawdown  = current_ret - peak
    threshold = STRATEGY["mdd_alert_threshold"]
    if drawdown < threshold:
        log.warning(f"[MDD 경보] 고점 {peak:+.1f}% / 현재 {current_ret:+.1f}% / 낙폭 {drawdown:+.1f}%p")
        send_telegram(
...
def build_performance_brief(portfolio: dict) -> str:
    ...
    # [v4.7] MDD 경보 체크 + 이력 저장
    check_mdd_alert(weighted_ret)
    save_performance_record("performance_check", weighted_ret, spy_ret, qqq_ret)

    # [v4.7] 성과 이력에서 고점·낙폭 계산 (브리핑 표시용)
    hist_records = load_performance_history()
    hist_rets    = [r["portfolio_ret_pct"] for r in hist_records if r.get("portfolio_ret_pct") is not None]
    peak_ret     = max(hist_rets) if hist_rets else weighted_ret
...
def _do_monthly_scan() -> None:
    ...
    try:
        ...
        brief = build_rebalancing_brief(
            ...
            prev_portfolio=prev_portfolio,   # [v4.6] 체크리스트용
        )
...
if __name__ == "__main__":
    ...
    log.info("✅ 미국주식 장기 투자 스캐너 v4.7 시작")
