...
def _cmd_status(chat_id: str) -> None:
    """/status — 현재 포트폴리오 수익률 + 낙폭 즉시 조회."""
    portfolio = load_portfolio()
    if not portfolio or not portfolio.get("holdings"):
        send_telegram("📭 보유 포트폴리오 없음 — 리밸런싱 후 다시 시도하세요")
        return
    ...
    holdings  = [h for h in portfolio["holdings"] if h["ticker"] != "CASH"]
    lines     = [
    ...
    for h in sorted(holdings, key=lambda x: -x["weight"]):
        lines.append(f"  {h['ticker']} {h['weight']}% | 진입가 ${h['entry_price']:,.2f}")
    _reply_to(chat_id, "\n".join(lines))
...
if __name__ == "__main__":
    ...
    log.info("✅ 미국주식 장기 투자 스캐너 v4.9 시작")
